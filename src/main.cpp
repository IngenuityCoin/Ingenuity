...
#if defined(NDEBUG)
#error "Ingenuity cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uingy) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
	...
	if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
		return error("%s : zINGY spend with serial %s is already in block %d\n", __func__,
...
{
	//Check to see if the zINGY is properly signed
	if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
		if (!spend.HasValidSignature())
			return error("%s: V2 zINGY spend does not have a valid signature", __func__);

		libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
		if (tx.IsCoinStake())
			expectedType = libzerocoin::SpendType::STAKE;
		if (spend.getSpendType() != expectedType) {
			return error("%s: trying to spend zINGY without the correct spend type. txid=%s", __func__,
				tx.GetHash().GetHex());
		}
	}
	...
	if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
		...
		return error("%s : zINGY spend with serial %s from tx %s is not in valid range\n", __func__,
...

	{
		...
		if (tx.IsZerocoinSpend()) {
			...
			if (IsTransactionInChain(tx.GetHash(), nHeightTx))
				return state.Invalid(error("AcceptToMemoryPool : zINGY spend tx %s already in block %d",
			...
			for (const CTxIn& txIn : tx.vin) {
				...
				if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
					return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
						tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zingy");
			}
		}
		else {
			LOCK(pool.cs);
			...
			// Check that zINGY mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight <= 30000 && nHeight > Params().LAST_POW_BLOCK()) {
		nSubsidy = 3 * COIN;
	}
	...
	else if (nHeight <= 200000 && nHeight > 130000) {
		nSubsidy = 3.25 * COIN;
	}
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZINGYStake)
{
	int64_t ret = 0;
	...
	if (nHeight < Params().LAST_POW_BLOCK()) {
		...
		ret = blockValue * 11 / 20;  //55%;
	}
	else if (nHeight <= 70000 && nHeight > 50000) {
		ret = blockValue * 3 / 5;  //60%;
	}
	else if (nHeight <= 80000 && nHeight > 70000) {
		ret = blockValue * 13 / 20;  //65%;
	}
	else if (nHeight <= 90000 && nHeight > 80000) {
	...
	//When zINGY is staked, masternode only gets 1 INGY
	if (isZINGYStake)
		ret = 1 * COIN;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
	...
	for (int i = block.vtx.size() - 1; i >= 0; i--) {
		...
		* note we only undo zerocoin databasing in the following statement, value to and from Ingenuity
...
{
	RenameThread("ingenuity-scriptch");
	scriptcheckqueue.Thread();
}

void RecalculateZINGYMinted()
{
...
void RecalculateZINGYSpent()
{
	...
	while (true) {
		...
		//Remove spends from zINGY supply
...
bool RecalculateINGYSupply(int nHeightStart)
{
...
bool UpdateZINGYSupply(const CBlock& block, CBlockIndex* pindex)
{
	std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
	...
	if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
		...
		RecalculateINGYSupply(Params().Zerocoin_StartHeight());
	}

	//Track zINGY money supply in the block index
	if (!UpdateZINGYSupply(block, pindex))
		return state.DoS(100, error("%s: Failed to calculate new zINGY supply for block=%s height=%d", __func__,
	...
	//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zIngySpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
	...
	// If turned on AutoZeromint will automatically convert INGY to zINGY
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
	...
	for (const CTransaction& tx : block.vtx) {
		...
		// double check that there are no double spent zINGY spends in this block
		if (tx.IsZerocoinSpend()) {
			for (const CTxIn& txIn : tx.vin) {
				if (txIn.scriptSig.IsZerocoinSpend()) {
					libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
					if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
						return state.DoS(100, error("%s : Double spending of zINGY serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
	if (nHeight < Params().Zerocoin_Block_V2_Start())
		return error("%s: zINGY stake block is less than allowed start height", __func__);

	if (CZIngyStake* zINGY = dynamic_cast<CZIngyStake*>(stake)) {
		CBlockIndex* pindexFrom = zINGY->GetIndexFrom();
		if (!pindexFrom)
			return error("%s: failed to get index associated with zINGY stake checksum", __func__);

		if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
			return error("%s: zINGY stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
		...
		uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zINGY->GetValue()));
		if (nChecksum200 != zINGY->GetChecksum())
			return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zINGY->GetChecksum(), nChecksum200);
	}
	else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
	...
	if (block.IsProofOfStake()) {
		...
		if (stake->IsZINGY() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
			return state.DoS(100, error("%s: staked zINGY fails context checks", __func__));
	...
	if (isPoS) {
		...
		// Inputs
		std::vector<CTxIn> ingyInputs;
		std::vector<CTxIn> zINGYInputs;

		for (const CTxIn& stakeIn : stakeTxIn.vin) {
			if (stakeIn.scriptSig.IsZerocoinSpend()) {
				zINGYInputs.push_back(stakeIn);
			}
			else {
				ingyInputs.push_back(stakeIn);
			}
		}
		const bool hasINGYInputs = !ingyInputs.empty();
		const bool hasZINGYInputs = !zINGYInputs.empty();
		...
		for (const CTransaction& tx : block.vtx) {
			for (const CTxIn& in : tx.vin) {
				...
				if (hasINGYInputs)
					...
					for (const CTxIn& ingyIn : ingyInputs) {
						if (ingyIn.prevout == in.prevout) {
		...
		if (isBlockFromFork) {
			...
			do {
				...
				for (const CTransaction& t : bl.vtx) {
					for (const CTxIn& in : t.vin) {
						...
						for (const CTxIn& stakeIn : ingyInputs) {
			...
			// Now that this loop if completed. Check if we have zINGY inputs.
			if (hasZINGYInputs) {
				for (const CTxIn& zIngyInput : zINGYInputs) {
					CoinSpend spend = TxInToZerocoinSpend(zIngyInput);
					...
					if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, spend, pindex, 0))
						return state.DoS(100, error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
							stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zingy");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
	...
	if (nMints || nSpends)
		LogPrintf("%s : block contains %d zINGY mints and %d zINGY spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
	...
	if (strCommand == "version") {
		...
		// Ingenuity: We use certain sporks during IBD, so check to see if they are
