// Copyright (c) 2018 The Ingenuity developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <primitives/deterministicmint.h>
#include "zingytracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzINGYTracker::CzINGYTracker(std::string strWalletFile)
{
...
CzINGYTracker::~CzINGYTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzINGYTracker::Init()
{
...
bool CzINGYTracker::Archive(CMintMeta& meta)
{
...
bool CzINGYTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzINGYTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzINGYTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzINGYTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzINGYTracker::GetSerialHashes()
{
...
CAmount CzINGYTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzINGYTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzINGYTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzINGYTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzINGYTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzINGYTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzINGYTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzINGYTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzINGYTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzINGYTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzINGYTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzINGYTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzINGYTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzINGYTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzINGYTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzINGYTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzINGYTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzingy from DB\n", __func__, listDeterministicDB.size());
    }
