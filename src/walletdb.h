...
public:
    ...
    bool WriteZINGYSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZINGYSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZINGYSeed_deprecated(uint256& seed);
    bool EraseZINGYSeed();
    bool EraseZINGYSeed_deprecated();

    bool WriteZINGYCount(const uint32_t& nCount);
    bool ReadZINGYCount(uint32_t& nCount);
