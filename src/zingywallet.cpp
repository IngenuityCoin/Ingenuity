...
CzINGYWallet::CzINGYWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zingy seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZINGYSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZINGYSeed_deprecated()) {
                    LogPrintf("%s: Updated zINGY seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zingy seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zingy wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzINGYWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZINGYCount(nCountLastUsed);
    else if (!walletdb.ReadZINGYCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzINGYWallet::Lock()
{
    seedMaster = 0;
}

void CzINGYWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzINGYWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZINGY(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzINGYWallet::LoadMintPoolFromDB()
{
...
void CzINGYWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzINGYWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzINGYWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zingyTracker->HasPubcoinHash(pMint.first)) {
...
bool CzINGYWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZINGY(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zingyTracker which also adds to database
    pwalletMain->zingyTracker->Add(dMint, true);
...
void CzINGYWallet::SeedToZINGY(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzINGYWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzINGYWallet::GenerateDeterministicZINGY(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzINGYWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzINGYWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
