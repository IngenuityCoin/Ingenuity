...
#ifndef Ingenuity_ZINGYWALLET_H
#define Ingenuity_ZINGYWALLET_H
...
public:
    CzINGYWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZINGY(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZINGY(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //Ingenuity_ZINGYWALLET_H
