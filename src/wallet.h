...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 INGY at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zINGY send
enum ZerocoinSpendStatus {
    ZINGY_SPEND_OKAY = 0,                            // No error
    ZINGY_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZINGY_WALLET_LOCKED = 2,                         // Wallet was locked
    ZINGY_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZINGY_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZINGY_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZINGY_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZINGY_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZINGY_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZINGY_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZINGY_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZINGY_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZINGY_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZINGY_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZINGY_SPENT_USED_ZINGY = 14,                      // Coin has already been spend
    ZINGY_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZINGY_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZIngyBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZINGYOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzingyAuto) const;
    ...
    */
    ...
    CzINGYWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzINGYTracker> zingyTracker;
    ...
    void setZWallet(CzINGYWallet* zwallet)
    {
        zwalletMain = zwallet;
        zingyTracker = std::unique_ptr<CzINGYTracker>(new CzINGYTracker(strWalletFile));
    }

    CzINGYWallet* getZWallet() { return zwalletMain; }
    ...
    void setZIngyAutoBackups(bool fEnabled)
    {
    ...
    /** zINGY reset */
    boost::signals2::signal<void()> NotifyzINGYReset;
