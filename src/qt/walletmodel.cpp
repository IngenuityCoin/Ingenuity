...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered ingenuity address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 INGY.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal ingenuity:URI (ingenuity:XyZ...?message=example)
            {
...
static void NotifyzINGYReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzINGYReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzINGYReset.connect(boost::bind(NotifyzINGYReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzINGYReset.disconnect(boost::bind(NotifyzINGYReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zingyTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
