...
#include "ui_zingycontroldialog.h"
...
std::set<std::string> ZIngyControlDialog::setSelectedMints;
std::set<CMintMeta> ZIngyControlDialog::setMints;

ZIngyControlDialog::ZIngyControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZIngyControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZIngyControlDialog::~ZIngyControlDialog()
{
    delete ui;
}

void ZIngyControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZIngyControlDialog::updateList()
{
...
void ZIngyControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZIngyControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZIngy_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZIngyControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZIngyControlDialog::GetSelectedMints()
{
...
void ZIngyControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
