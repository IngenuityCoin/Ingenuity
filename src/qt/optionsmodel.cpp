...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::INGY);
    ...
    if (!settings.contains("nAnonymizeIngenuityAmount"))
        settings.setValue("nAnonymizeIngenuityAmount", 1000);

    nAnonymizeIngenuityAmount = settings.value("nAnonymizeIngenuityAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeIngenuityAmount"))
        SoftSetArg("-anonymizeingenuityamount", settings.value("nAnonymizeIngenuityAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in ingenuity.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeIngenuityAmount:
            return QVariant(nAnonymizeIngenuityAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeIngenuityAmount:
            nAnonymizeIngenuityAmount = value.toInt();
            settings.setValue("nAnonymizeIngenuityAmount", nAnonymizeIngenuityAmount);
            emit anonymizeIngenuityAmountChanged(nAnonymizeIngenuityAmount);
            break;
        case CoinControlFeatures:
